  ...
  if (ehSaudacaoSimples(mensagem) && sessao.historico.length === 0) {
    ...
    if (candidatoExistente?.encontrado) {
      ...
      sessao.historico.push({
        role: "assistant",
        content: "Candidato já cadastrado."
      });
...
async function buscarCandidatoNaPlanilha(telefone) {
  try {
    ...
    const url = `${urlBase}?acao=candidato&telefone=${encodeURIComponent(telefone)}`;

    const response = await axios.get(url, { timeout: 15000 });
    return response.data;
  } catch (erro) {
    console.error("Erro ao buscar candidato:", JSON.stringify(erro.response?.data || erro.message));
    return null;
  }
}
